...
class DownBlock(nn.Module):
    def __init__(self, inchannels, outchannels, dilation, pool=True):
        ...
        blk.append(nn.Conv2d(inchannels, outchannels,
                   kernel_size=3, padding=1, dilation=dilation))
        blk.append(nn.ReLU(inplace=True))
        blk.append(nn.Conv2d(outchannels, outchannels,
                   kernel_size=3, padding=1, dilation=dilation))
        blk.append(nn.ReLU(inplace=True))
...
class UpBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x1 = F.interpolate(
            x1, size=(x2.size()[2], x2.size()[3]), mode='nearest')
...
class Network(nn.Module):
    ...
    def build_model(self, warp1_tensor, warp2_tensor, mask1_tensor, mask2_tensor):
        out = self.forward(warp1_tensor, warp2_tensor)

        learned_mask1 = (mask1_tensor - mask1_tensor *
                         mask2_tensor) + mask1_tensor*mask2_tensor*out
        learned_mask2 = (mask2_tensor - mask1_tensor *
                         mask2_tensor) + mask1_tensor*mask2_tensor*(1-out)
        stitched_image = (warp1_tensor+1.) * learned_mask1 + \
            (warp2_tensor+1.)*learned_mask2 - 1.

        out_dict = {}
        out_dict.update(learned_mask1=learned_mask1,
                        learned_mask2=learned_mask2, stitched_image=stitched_image)
